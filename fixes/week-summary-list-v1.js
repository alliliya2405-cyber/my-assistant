...
(() => {
  ...
  function tasksForWeek(){
    ...
    return (state.tasks||[]).filter(t=>t.date>=start&&t.date<=end&&matches(t,selectedFilter)).slice().sort((a,b)=>(a.date||'').localeCompare(b.date||'')||(a.start||'99:99').localeCompare(b.start||'99:99')||(a.title||'').localeCompare(b.title||'','ru'));
  }
  ...
  function decorateSummary(){
    ...
    [...summary.children].forEach((item,index)=>{
      const filter=filters[index];if(!filter)return;
      item.dataset.weekSummaryFilter=filter;
      item.setAttribute('role','button');
      item.setAttribute('tabindex','0');
      item.setAttribute('aria-pressed',String(listRequested&&selectedFilter===filter));
      item.classList.toggle('active',listRequested&&selectedFilter===filter);
    });
  }

  function renderList(){
    const workspace=document.querySelector('.week-workspace');
    if(!workspace)return;
    decorateSummary();
    const old=workspace.querySelector('[data-week-summary-list]');
    ...
    if(!old)workspace.querySelector('.week-summary')?.insertAdjacentElement('afterend',section);
    decorateSummary();
  }
  ...
  document.addEventListener('click',event=>{
    const summaryItem=event.target.closest('.week-summary [data-week-summary-filter]');
    if(summaryItem){event.preventDefault();activateFilter(summaryItem.dataset.weekSummaryFilter);return}
    const close=event.target.closest('[data-week-list-close]');
    if(close){event.preventDefault();event.stopPropagation();listRequested=false;close.closest('[data-week-summary-list]')?.remove();decorateSummary();return}
    const add=event.target.closest('[data-week-list-add]');
    if(add){event.preventDefault();event.stopPropagation();if(typeof openTask==='function')openTask(null,{date:add.dataset.date||bounds()[0],start:add.dataset.start||'09:00'});return}
    const edit=event.target.closest('[data-week-list-edit]');
    if(edit){const task=(state.tasks||[]).find(t=>t.id===edit.dataset.weekListEdit);if(task&&typeof openTask==='function')openTask(task);return}
    const done=event.target.closest('[data-week-list-done]');
